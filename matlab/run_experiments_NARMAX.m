...
addpath(genpath("ILS-estimator-NARMAX"));
addpath(genpath("realizations"));

%%

% Series of train sizes
% trn_sizes = 2.^[6:10];
trn_sizes = [100,200,400,800];
...
while r <= num_repeats
    ...
    % Generate signal
     ...
     if (max(abs(yTrain)) < 100) && (sum(isnan(yTrain))==0)
        ...
        save("realizations/NARMAXsignal_stde"+num2str(options.stde)+"_degree"+num2str(options.nd)+"_delays"+num2str(M_m)+"_D"+num2str(N_m)+"_r" + string(r) + ".mat", "yTrain", "yTest", "uTrain", "uTest", "eTrain", "eTest", "system", "options")
%         load("generated/NARMAXsignal_stde"+num2str(options.stde)+"_degree"+num2str(options.nd)+"_delays"+num2str(M_m)+"_D"+num2str(22)+"_r" + string(r) + ".mat", "yTrain", "yTest", "uTrain", "uTest", "eTrain", "eTest", "system", "options")
        % Preallocate result arrays
...
%             RMS_sim(n) = rms(dataTest.y - ySimIterTest);
        ...
        save("ils_results/results-NARMAX_ILS_stde"+num2str(options.stde)+"_pol"+num2str(options.nd)+"_delays"+num2str(M_m)+"_D"+num2str(22)+"_degree3_r"+num2str(r)+".mat", "RMS_prd", "RMS_sim")
